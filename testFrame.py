...
parser.add_argument('--backend_target', '-bt', type=int, default=0, help='''Choose one of the backend-target pair to run this demo:
                        {:d}: (default) CUDA + GPU (CUDA FP16),
                        {:d}: CUDA + GPU (CUDA),
                        {:d}: TIM-VX + NPU,
                        {:d}: CANN + NPU
...
# @jit(parallel=True)
# def main():
#     print("Is CUDA supported by this system?:" f'{torch.cuda.is_available()}')
    
#     print(f"CUDA version:", f'{torch.version.cuda}')
